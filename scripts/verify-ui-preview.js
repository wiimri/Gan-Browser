...
function verifyInternalPageRoutes() {
  ...
  const requirements = [
    ...
    [internalPages.includes("gxlight:passwords:view:"), "password vault entries cannot be viewed securely"],
    [internalPages.includes("Importar CSV/TXT"), "password vault import is not available from privacy settings"],
    [internalPages.includes("passwordAction") && internalPages.includes("gxlight:passwords:"), "password vault import action is not wired in settings"],
    [!browserForm.includes("ToolStripMenuItem passwords = new ToolStripMenuItem"), "password vault still appears as a duplicated top-level menu"],
    [browserForm.includes("WindowsHelloVerifier.VerifyAsync"), "password vault actions are not protected by Windows Hello"],
    [browserForm.includes("PasswordVaultSecurity.MatchesExactHost"), "password vault autofill does not require an exact host match"],
    [internalPages.includes("Rellenar sitio actual"), "password vault autofill action is missing from privacy settings"],
